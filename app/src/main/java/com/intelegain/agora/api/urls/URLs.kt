package com.intelegain.agora.api.urls

/***
 * This is Constant
 * class used for declare variable's that is globally used
 * anywhere in the application.Here all the static variable's are declared and
 * used anywhere in application.
 */
object URLs {
    //public static final String CUSTOMERBASE_URL = "http://10.0.0.2:171/AgoraMobile.api/";
    const val CUSTOMERBASE_URL = "http://www.agoracust.sirus/AgoraMobile.api/"
    ////////////Sirus New URL
//public static final String BASE_URL = "http://168.63.218.137//AgoraMobile.api/";  ////Straiging URL
    const val BASE_URL = "http://emp.intelgain.com/AgoraMobile.api/" ///Live URL
    //sirus url
//public static final String BASE_URL = "http://10.0.0.2:170/AgoraMobile.api/";
//staging url
//    public static final String BASE_URL = "http://52.172.192.203/AgoraMobile.api/";
}